...
# --------------------------------------------文件存储-----------------------------------------------------
path = os.getcwd() + "/static/csv/weiboComments.csv"
...
def get_title_id():
    for page in range(0, 21):  # 每个页面大约有18个话题
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.100 Safari/537.36"
            # chrome浏览器随机代理
        }
...
def weibo_word_cloud():
    ...
    #     print("%s:%d"%(k,v))
    # 存储数据
    name = "./static/csv/weibo-words-data.csv"
...
def weibo_posneg_analysis():
    ...
    for i in words:
        ...
        # print(positive)
    dataframe = pd.DataFrame({'positive': positive, 'negative': negative, 'neutral': neutral})
    dataframe.to_csv("static/csv/weibo-evaluation-results.csv", index=False, sep=',', header=True)
